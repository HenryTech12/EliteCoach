...
function LearningPathPage() {
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    try {
      const targetRole = goal || normalizeUserType(user?.userType) || "Career growth";
  ...
  const saveGoal = async () => {
    ...
    try {
      await Promise.all([
        ...
        acsApi.patch("/v1/assessment/profile", {
          career_goal: goal,
      ...
      setPath((p) => ({ ...(p ?? {}), goal, target_role: goal, time_per_week: hours }));
      setEditing(false);
        ...
        {editing && (
          <div className="fixed inset-0 bg-navy/50 z-50 flex py-12 justify-center px-4 overflow-y-auto">
            <div className="bg-surface w-full max-w-md p-6 relative rounded-sm h-fit">
              ...
              <h2 className="text-xl font-bold mb-6 mt-2">Edit goal</h2>
        ...
        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          <div>
            ...
            <h1 className="text-4xl font-bold tracking-tight">
              {path?.target_role || path?.goal || "Build your learning path"}
            </h1>
            {path?.time_per_week && (
          ...
          <div className="card-base text-center py-16">
            ...
            <div className="card-base font-mono whitespace-pre-wrap text-sm leading-relaxed overflow-x-auto text-navy">
                ...
                <div className="mt-8 pt-4 border-t border-border flex justify-between items-center">
                    <p className="text-base font-semibold">Next up: <span className="text-primary">{path.next_course || "N/A"}</span></p>
                </div>
            </div>
        ) : (
